...
func (qt *quotaTopology) validateQuotaTopology(oldQuotaInfo, newQuotaInfo *QuotaInfo, oldNamespaces []string) error {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.ElasticEnableResourceTypeUpdate) {
		enableResourceTypeUpdate = true
	}
	if err := qt.checkSubAndParentGroupQuotaKey(newQuotaInfo, enableResourceTypeUpdate); err != nil {
		...
		if child, exist := qt.quotaInfoMap[name]; exist {
			if enableResourceTypeUpdate {
				if !checkQuotaKeyIncluded(quotaInfo.CalculateInfo.Max, child.CalculateInfo.Max) {
					return fmt.Errorf("%v's max key is not included with %v",
						name, quotaInfo.Name)
				}
			} else {
